...
public class Simulation {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            for (double p_graph : new double[]{.1,.2,.3}) {
                for (double p_malicious : new double[]{.15, .3, .45}) {
                    for (double p_txDistribution : new double[]{.01, .05, .1}) {
                        for (int numRounds : new int[]{10, 20}) {
                            startSimulation(numNodes, p_graph, p_malicious, p_txDistribution, numRounds);
                        }
                    }
                }
            }
    ...
    private static void startSimulation(int numNodes, double p_graph, double p_malicious, double p_txDistribution, int numRounds) {
        ...
        simulate(numNodes, numRounds, followees, nodes, validTxIds);
    ...
    public static void simulate(int numNodes, int numRounds, boolean[][] followees, Node[] nodes, HashSet<Integer> validTxIds) {
            // Simulate for numRounds times
